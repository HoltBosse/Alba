	...
	public function display() {
		...
		echo "<div class='field {$required} {$hidden}'>";
			...
			echo "<div class='control " . ($this->icon_status ? $this->icon_parent_class : false) . "'>";
				...
				echo "<input type='{$this->input_type}' value='{$value}' placeholder='{$placeholder}' {$minmax} {$pattern} {$step} {$this->get_rendered_name()} {$this->get_rendered_form()} maxlength={$this->maxlength} minlength={$this->minlength} class='filter_{$this->filter} input' {$required} type='text' id='{$this->id}' >";
