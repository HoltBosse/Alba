...
class Field_Video extends Field {
	...
	public function display($repeatable_template=false) {
		...
		echo "<style>.selected_image_wrap img {max-width: 40rem;}</style>";
		...
		trigger_image_selector_<?php echo $this->id; ?>.addEventListener('click',function(e){
			...
			function fetch_images(searchtext, taglist) {
				...
				// fetch images
				postAjax('<?php echo Config::uripath();?>/admin/videos/api', {"action":"list_videos","searchtext":searchtext}, function(data) { 
					...
					image_list.videos.data.forEach(image => {
						let vdata = {
							thumb_with_play_button: image.pictures.sizes[(image.pictures.sizes.length-1)].link_with_play_button,
							thumb: image.pictures.sizes[(image.pictures.sizes.length-1)].link,
							embed: image.embed.html,
							video: image.files[0].link
						};
						image_list_markup += `
						<li>
							<a class='media_selector_selection' data-vdata='${JSON.stringify(vdata)}'>
							<img src='${image.pictures.sizes[(image.pictures.sizes.length-1)].link_with_play_button}'>
					...
					media_selector.addEventListener('click',function(e){
						...
						if (selected_image!==null) {
							...
							preview.src = JSON.parse(selected_image.dataset.vdata).thumb_with_play_button;
							preview.closest('.selected_image_wrap').classList.add('active');

							hidden_input = document.getElementById('<?php echo $this->id;?>');
							hidden_input.value = selected_image.dataset.vdata;
