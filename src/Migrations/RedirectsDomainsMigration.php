...
class RedirectsDomainsMigration extends Migration {
    public function isNeeded(): Message {
        if($this->status == null) {
            $result = DB::fetchall("SELECT DISTINCT domain FROM redirects WHERE domain NOT REGEXP '^[0-9]+$'");
            if(!$result) {
                $this->status = new Message(false, MessageType::Warning, "Redirects domain column missing");
            } else {
                $this->status = new Message(true, MessageType::Success, "Redirects domain column exists");
            }
        }
