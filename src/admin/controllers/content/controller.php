...
foreach (Content::getContentControllerNames() as $f) {
	foreach ($all_content_types as $existing_content_type) {
		if ($f==$existing_content_type->controller_location) {
	...
	<div class="message-body">
		...
		<ul class='list'>
			...
			<?php foreach ($missing as $missed) {
				$controller_config_file = Content::getContentControllerPath($missed) . '/controller_config.json';
				$custom_fields_file = Content::getContentControllerPath($missed) . '/custom_fields.json';
				if (is_readable($controller_config_file)) {
					...
					if (!$w_config || !$w_config->title || !$custom_fields->id) {
						...
						if ($ok) {
							...
							$view_folders=[];
							foreach (new DirectoryIterator(Content::getContentControllerPath($missed) . "/views/") as $f) {
							...
							foreach ($view_folders as $view_folder) {
								$view_config_file = Content::getContentControllerPath($missed) . '/views/' . $view_folder . '/view_configuration.json';
